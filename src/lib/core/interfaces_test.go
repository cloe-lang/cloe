...
func TestEqualTrue(t *testing.T) {
	for _, vs := range [][]Value{
		...
		assert.True(t, bool(*EvalPure(PApp(Equal, vs...)).(*BooleanType)))
	}
}
...
func TestIsOrdered(t *testing.T) {
	for _, v := range []Value{
		...
		assert.True(t, bool(*EvalPure(PApp(IsOrdered, v)).(*BooleanType)))
	}
	...
	for _, v := range []Value{
		...
		assert.False(t, bool(*EvalPure(PApp(IsOrdered, v)).(*BooleanType)))
	}
}
